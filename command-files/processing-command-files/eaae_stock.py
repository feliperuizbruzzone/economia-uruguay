"""Shared fixed-asset stock extraction helpers for the EAAE pipeline."""
...
def find_stock_member(year: int, members: list[str]) -> str | None:
    config = EAAE_STOCK_CONFIG[year]
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"Year {year}: expected exactly one fixed-asset stock XLS matching "
            f"{pattern_value!r}, found {matches}"
        )
    return matches[0]


def read_stock_source_rows(year: int, xls_path: Path) -> list[dict[str, object]]:
    config = EAAE_STOCK_CONFIG[year]
    ...
    if configured_start is not None and detected_start != configured_start:
        LOGGER.warning(
            "Year %s: detected stock data_start_row=%s differs from config=%s",
            year,
            detected_start,
            configured_start,
        )
    ...
    stock_col = int(config["stock_capital_col"])
    ...
    for row_idx in range(detected_start, sheet.nrows):
        ...
        if not re.fullmatch(r"[A-Z]", section):
            continue
        value = to_number(sheet.cell_value(row_idx, stock_col))
        row: dict[str, object] = {
            "seccion_fuente": section,
            "stock_capital": value * scale if value is not None else None,
        }
    ...
    if not rows:
        raise RuntimeError(f"Year {year}: no stock rows extracted from {xls_path}")
    return rows


def extract_stock_year(
    year: int, unrar_bin: str | None = None
) -> list[dict[str, object]]:
    source_config = EAAE_CONFIG[year]
    stock_config = EAAE_STOCK_CONFIG[year]
    if stock_config["file_pattern"] is None:
        LOGGER.info(
            "Year %s: no configured fixed-asset stock source; leaving stock empty",
            year,
        )
        return []
    ...
    member = find_stock_member(year, members)
    if member is None:
        return []
    LOGGER.info("Year %s: extracting fixed-asset stock from %s", year, member)

    with tempfile.TemporaryDirectory(prefix=f"eaae-stock-{year}-") as tmpdir:
        xls_path = extract_member(rar_path, member, Path(tmpdir), unrar)
        source_rows = read_stock_source_rows(year, xls_path)
    ...
    for row in select_additive_rows(source_rows):
        ...
        value = row.get("stock_capital")
        if value is not None:
            sums[section] += float(value)
            present.add(section)

    return [
        {"anno": year, "seccion": section, "stock_capital": sums[section]}
        for section in sorted(present)
    ]


def extract_stock_panel(years: list[int] | None = None) -> list[dict[str, object]]:
    selected_years = years if years is not None else PANEL_YEARS
    unrar = find_unrar()
    records: list[dict[str, object]] = []
    for year in selected_years:
        records.extend(extract_stock_year(year, unrar))
    return records


def validate_stock_year(year: int, rows: list[dict[str, object]]) -> None:
    if year in STOCK_MISSING_YEARS:
        if rows:
            raise AssertionError(f"Year {year}: unexpected stock_capital rows")
        return
    ...
    if not sections.issuperset(CIIU_HOMOLOGATED_MINIMUM_SECTIONS):
        ...
        raise AssertionError(f"Year {year}: missing stock_capital sections {missing}")
